...
function disableFrame(frame) {
  if (frame._src != 'iframeEmpty.html')
...
function enableFrame(frame) {
  ...
  if (frame.src != src && src != 'about:blank' && src != 'iframeEmpty.html') {
    frame.src = src;
  }
};
...
function handleBodyKeyDown(event) {
  switch (event.keyCode) {
	...
	//adon
    case 80: case 229: // print
	  nextSlide = function(){};
	  prevSlide = function(){};
	
	document.querySelector('section.slides').classList.add('print');
	document.body.style.overflow = 'visible';
	document.body.style.background = 'none';

	// add page number  
	var articles = document.querySelectorAll('section.slides article'), text = document.getElementById('footer') ? document.getElementById('footer').innerHTML : '';
	for (var i = 0, div, article; article = articles[i]; i++) {
		div = document.createElement('div');
		div.className = "pagenum fitPageWidth";
	  ...
	  for (var i = 0, len = slideEls.length; i<len; i++) {
		  triggerEnterEvent(i);
	  }
...
function addDemoHTML() {
  ...
  for (var i = 0, el; el = els[i]; i++) {
	if (!(iframe = el.querySelector('iframe'))) {
		iframe = document.createElement('iframe');
		iframe.src = iframe._src = 'incl/iframeEmpty.html';
		el.appendChild(iframe);
		do {
			el = el.parentNode;
		} while (el.tagName != 'ARTICLE');
		el.addEventListener('slideenter', function(){
		  for (var i = 0, pre, els = this.querySelectorAll('section.demoHTML pre'); pre = els[i]; i++) {
			  if (pre && 'true' != pre.contentEditable) {
				...
				(evt.initKeyEvent)
					? evt.initKeyEvent('keyup', true, true, document.defaultView, 
									  false, false, false, false,
									  13, 0)
					: evt.initKeyboardEvent('keyup', true, true, document.defaultView, "Enter", 0, null);
				evt.keyCode = 13;
